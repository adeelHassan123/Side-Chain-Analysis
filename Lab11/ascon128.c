/**
 * ascon128.c – Minimal but complete ASCON-128 encryption (AEAD)
 * For ARM Cortex-M3, side-channel analysis lab.
 * No standard library dependencies (except stdint.h for types).
 */
...
/* -------------------------------------------------------------------------
 * 1. State representation: five 64-bit words
 * ------------------------------------------------------------------------- */
typedef struct {
    uint64_t x[5];
} ascon_state_t;
...
/* -------------------------------------------------------------------------
 * 2. ASCON S-box (5-bit to 5-bit) – Lookup table for simplicity
 *    Values from ASCON specification.
 * ------------------------------------------------------------------------- */
...
/* Apply S-box to the whole 320-bit state (5 words of 64 bits)
 * Each word is processed in 5-bit chunks (least significant bits first).
 */
...
/* -------------------------------------------------------------------------
 * 3. Linear diffusion layer (rotation and XOR)
 * ------------------------------------------------------------------------- */
...
/* -------------------------------------------------------------------------
 * 4. Add round constant (only to x2)
 * ------------------------------------------------------------------------- */
...
/* -------------------------------------------------------------------------
 * 5. ASCON permutation p12 (12 rounds) – also used for p6, p1 etc.
 *    Round constants for first 12 rounds.
 * ------------------------------------------------------------------------- */
...
/* -------------------------------------------------------------------------
 * 6. Helper: convert 8 bytes to 64-bit big-endian
 * ------------------------------------------------------------------------- */
...
/* -------------------------------------------------------------------------
 * 7. Initialization (ASCON-128)
 *    Input: 16-byte key, 16-byte nonce
 * ------------------------------------------------------------------------- */
static void ascon_init(uint64_t state[5], const uint8_t key[16], const uint8_t nonce[16]) {
    // Initialization constant for ASCON-128 (rate = 64 bits)
    ...
    ascon_permutation(state, 12);  // p12
}

/* -------------------------------------------------------------------------
 * 8. Encryption of a single 8-byte block (full block)
 *    XOR plaintext with state[0], then feedback ciphertext into state[0]
 *    and apply p6 permutation.
 * ------------------------------------------------------------------------- */
static void ascon_encrypt_block(uint64_t state[5], uint8_t *ciphertext,
                                const uint8_t *plaintext) {
    uint64_t p = bytes_to_u64be(plaintext);
    uint64_t c = state[0] ^ p;
    u64_to_bytes_be(c, ciphertext);
    state[0] = c;
    ascon_permutation(state, 6);  // p6
}

/* -------------------------------------------------------------------------
 * 9. Finalization: produce 16-byte authentication tag
 *    Apply p12 then extract first 128 bits of state.
 * ------------------------------------------------------------------------- */
...
/* -------------------------------------------------------------------------
 * 10. Main AEAD encryption (simplified – no associated data, only plaintext)
 *     Handles any length, pads last block with zeros.
 * ------------------------------------------------------------------------- */
void ascon_encrypt(const uint8_t key[16], const uint8_t nonce[16],
    ...
    // Partial block (padding with zeros)
    size_t rem = len % 8;
    if (rem > 0) {
